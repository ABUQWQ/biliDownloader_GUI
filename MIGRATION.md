...
- **暗色漏网控件**：把 `QSpinBox`、树表头、勾选框指示器、滚动条和 `QMessageBox` 也锁成白底黑字，避免系统暗色下黑字贴暗底。
- **互动视频探查与下载**：JSON 导出改为写入节点树并使用 UTF-8；缺省缩略图回退到内置图标；空选择下载会被拦截；默认探查深度改为 3；新增“一键探查全部节点”，下载前会确认节点数量。

...
class biliWorker(QThread):
    ...
    def search_preinfo(self, index_url):
        try:
            source = parse_source(index_url, self.api_options)
            media = get_media_info(source, self.api_options)
            streams = get_stream_options(media, self.api_options)
            self.current_media = media
            title, duration, download_data = legacy_detail(media, streams)
            selected = next((page for page in media.pages if page.cid == media.cid), None)
            if selected and selected.title and selected.title != title:
                title = "{}_[{}]".format(title, selected.title)
            return 1, self.name_replace(title), duration, download_data
        except Exception as e:
            print("[EXCEPTION]BiliWorker.main.biliWorker.search_preinfo:", e)
            self.business_info.emit("获取视频信息失败：{}".format(e))
            return 0, "", "", {}
    ...
    def search_videoList(self, index_url):
        try:
            ...
            media = get_media_info(source, self.api_options)
            self.current_media = media
            current_page = next((page.index for page in media.pages if page.cid == media.cid), 1)
            if media.is_bangumi:
                pages = [
                    {
                        "link": page.url,
                        "share_copy": page.title,
                        "cid": page.cid,
                        "ep_id": page.episode_id,
                    }
                    for page in media.pages
                ]
                return 2, {"bvid": source.identifier, "p": current_page, "pages": pages}
            pages = [
                {"page": page.index, "part": page.title, "cid": page.cid}
                for page in media.pages
            ]
            return 1, {"bvid": media.bvid or media.avid, "p": current_page, "pages": pages}
        except Exception as e:
    ...
    def d_processor(self, url_list, output_dir, output_file, dest):
        for line in filter(None, url_list):
            self.business_info.emit('使用线路：{}'.format(line.split("?")[0]))
            try:
                probe_headers = dict(self.second_headers)
                ...
                content_range = video_bytes.headers.get('Content-Range', '')
                content_length = video_bytes.headers.get('Content-Length', '0')
                vc_range = content_range.rsplit('/', 1)[-1] if '/' in content_range else content_length
                total_size = int(vc_range or 0)
                video_bytes.close()
                if total_size <= 0:
                    raise DownloadError('服务器没有返回有效的流长度')
                self.business_info.emit("获取{}流范围为：{}".format(dest, vc_range))
                self.business_info.emit('{}  文件大小：{} MB'.format(dest, round(total_size / 1024 / 1024, 4)))
                existing_size = os.path.getsize(output_file) if os.path.exists(output_file) else 0
                if existing_size > total_size:
                    os.remove(output_file)
                    existing_size = 0
                proc = {"Max": total_size, "Now": existing_size, "finish": 0}
                if existing_size == total_size:
                    proc["finish"] = 1
                    self.progr_bar.emit(proc)
                    self.business_info.emit("{}已完成，跳过重复下载。".format(dest))
                    return 0
                err = 0
                while err <= self.set_err:
                    try:
                        download_headers = dict(self.second_headers)
                        download_headers['range'] = 'bytes={}-{}'.format(proc["Now"], total_size - 1)
                        ...
                        m4sv_bytes.raise_for_status()
                        if proc["Now"] and m4sv_bytes.status_code == 200:
                            proc["Now"] = 0
    ...
    def Download_single(self, index=""):
        ...
        if flag:
            try:
                ...
                video_item = down_dic["video"].get(self.VQuality) or down_dic["video"].get(0)
                if not video_item or not video_item[1]:
                    raise DownloadError("所选视频流不可用")
                self.second_headers['range'] = video_item[2]
                # Switch between main line and backup line(video).
                if self.killprocess:
                    return -2
                a = self.d_processor(video_item[1], self.output, video_dir, "下载视频")
                audio_item = down_dic["audio"].get(self.AQuality) or down_dic["audio"].get(0)
                has_audio = bool(audio_item and audio_item[1])
                b = 0
                if has_audio:
                    self.second_headers['range'] = audio_item[2]
                    if self.killprocess:
                        return -2
                    b = self.d_processor(audio_item[1], self.output, audio_dir, "下载音频")
                if a or b:
                    return -3
                # Merge audio and video (USE FFMPEG)
                if self.killprocess:
                    return -2
                if self.synthesis and has_audio:
                    self.business_info.emit('正在启动FFMPEG......')
                    self.ffmpeg_synthesis(video_dir, audio_dir, sym_video_dir)
                elif self.synthesis:
                    os.rename(video_dir, sym_video_dir)
            except Exception as e:
        ...
        if all_list[0] == 1:
            ...
            if r_list[0] == 0:
                for p in all_list[1]["pages"]:
                    if self.killprocess:
                        break
                    target = p["link"] or "https://www.bilibili.com/bangumi/play/ep{}".format(p.get("ep_id"))
                    self.Download_single(target)
            else:
                listLEN = len(all_list[1]["pages"])
                for i in r_list:
                    if self.killprocess:
                        break
                    if i <= listLEN:
                        page = all_list[1]["pages"][i - 1]
                        target = page["link"] or "https://www.bilibili.com/bangumi/play/ep{}".format(page.get("ep_id"))
                        self.Download_single(target)
                    else:
                        continue
    ...
    def Get_Init_Info(self, url):
        try:
            ...
            if media.is_bangumi or not media.bvid or not media.cid:
                raise DownloadError("交互视频只支持普通 BV 视频")
    ...
    def down_list_make(self, cid_num):
        try:
            media = MediaInfo(
                source=parse_source(self.index_url, self.api_options),
                title=self.now_interact.get("vname", "交互视频"),
                duration=0,
                bvid=self.now_interact["bvid"],
                cid=int(cid_num),
            )
            streams = get_stream_options(media, self.api_options)
            return True, streams.duration, {"video": streams.video, "audio": streams.audio}
        except Exception as e:
            return False, str(e)
